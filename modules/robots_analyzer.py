...
def analyze_robots_txt(
    ...
    logger.info(
        f"Starting robots.txt analysis for {target}"
    )
    ...
    parsed_data = parse_robots_content(
        robots_content
    )



    parsed_data["sensitive_paths"] = (
        detect_sensitive_paths(
            parsed_data["disallow"]
        )
    )



    parsed_data["sitemap_analysis"] = (
        analyze_sitemaps(
            parsed_data["sitemaps"]
        )
    )
